...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    ...
    // Insert new user
    $sql = "INSERT INTO registration (full_name, email, user_name, phone_number, dob, password, address) VALUES (?, ?, ?, ?, ?, ?, ?)";
    $stmt = $conn->prepare($sql);
    $stmt->bind_param("sssssss", $full_name, $email, $username, $phone_number, $dob, $password, $address);
    if ($stmt->execute()) {
        echo "<script>alert('✅ User account created successfully!'); window.location='manage_user.php';</script>";
    } else {
...
<head>
    <meta charset="UTF-8">
    <title>Add User - Meat Bazar</title>
    ...
    <div class="container">
        ...
        <div class="content">
            <div class="form-container">
                ...
                <form onsubmit="return validateRegister()" method="POST" action="add_user.php">
                    ...
                    <button type="submit" class="btn btn-create">Create User Account</button>
                </form>
                <div style="display: flex; justify-content: center;">
                    <button type="button" class="btn btn-back" onclick="window.location='manage_user.php'">Back</button>
                </div>
            </div>
        </div>
    </div>
