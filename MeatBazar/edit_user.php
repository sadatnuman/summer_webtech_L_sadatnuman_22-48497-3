...
// Handle user update by username
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['edit_username'])) {
    ...
    $sql = "UPDATE registration SET full_name = ?, phone_number = ?, dob = ?, address = ? WHERE user_name = ?";
    $stmt = $conn->prepare($sql);
    $stmt->bind_param("sssss", $full_name, $phone_number, $dob, $address, $username);

    if ($stmt->execute()) {
        echo "<script>alert('✅ User information updated successfully!'); window.location='edit_user.php';</script>";
    } else {
        echo "<script>alert('❌ Error updating user.'); window.location='edit_user.php';</script>";
    }
    $stmt->close();
}
...
<head>
    <meta charset="UTF-8">
    <title>Edit User - Meat Bazar</title>
                        ...
                        <tr>
                            ...
                            <td>
                                ...
                                <form method="POST" action="edit_user.php" style="display:inline;">
                                    <input type="hidden" name="edit_username" value="<?php echo $user['user_name']; ?>">
