...
// Handle distributor deletion by username
if (isset($_POST['delete_username']) && !empty($_POST['delete_username'])) {
    $username = $_POST['delete_username'];

    // Prepare the delete query using the username as the identifier
    $sql = "DELETE FROM distributor WHERE user_name = ?";
    $stmt = $conn->prepare($sql);
    $stmt->bind_param("s", $username);

    if ($stmt->execute()) {
        echo "<script>alert('✅ Distributor deleted successfully!'); window.location='delete_distributor.php';</script>";
    } else {
        echo "<script>alert('❌ Error deleting distributor.'); window.location='delete_distributor.php';</script>";
    }
...
<head>
    <meta charset="UTF-8">
    <title>Manage Distributor - Meat Bazar</title>
                ...
                <tbody>
                    ...
                    <?php foreach ($distributors as $distributor): ?>
                        <tr>
                            ...
                            <td>
                                <!-- Delete Button Form -->
                                <form method="POST" action="delete_distributor.php" style="display:inline;">
                                    <input type="hidden" name="delete_username" value="<?php echo $distributor['user_name']; ?>">
                                    <button type="submit" class="black-btn">Delete</button>
                                </form>
                            </td>
                        </tr>
                    <?php endforeach; ?>
