...
try {
    // Fetch all orders that are pending and not assigned
    $sql = "SELECT * FROM orders WHERE order_status = 'pending' AND (assigned_distributor IS NULL OR assigned_distributor = '') ORDER BY created_at DESC";
    ...
    $sql_distributors = "SELECT user_name, full_name FROM distributor";
    $result_distributors = $conn->query($sql_distributors);

    if ($result_distributors) {
        while ($row = $result_distributors->fetch_assoc()) {
            $distributors[] = $row;
        }
    } else {
        $error_message = "Error fetching distributors: " . $conn->error;
    }
} catch (Exception $e) {
...
<body>
    ...
    <div class="nav">
        ...
        <a href="admin_all_orders.php" >All Orders</a>
    ...
    <div class="container">
        ...
        <div class="content">
            <div class="orders-container">
                <h2 class="page-title">Assign Orders to Distributors</h2>
                ...
                <div class="orders-stats">
                    ...
                    <div class="stat-card">
                        ...
                        <p class="number"><?php echo count($distributors); ?></p>
                    </div>
                </div>

                <div class="orders-table">
                    <div class="table-header">
                        <i class="fa fa-shopping-cart"></i> Assign Orders to Distributors
                    </div>
                            ...
                            <?php foreach ($orders as $order): ?>
                                <div class="order-item">
                                    ...
                                    <div class="order-actions">
                                        <select class="distributor-select" id="distributor-<?php echo $order['id']; ?>">
                                            <option value="">Select a Distributor</option>
                                            <?php foreach ($distributors as $distributor): ?>
                                                <option value="<?php echo htmlspecialchars($distributor['user_name']); ?>">
                                                    <?php echo htmlspecialchars($distributor['full_name']); ?>
                                                </option>
                                            <?php endforeach; ?>
                                        </select>
                                        <button class="action-btn" onclick="assignOrder(<?php echo $order['id']; ?>)">
                                            Assign Order
                                        </button>
                                    </div>
                                </div>
    ...
    <script>
        ...
        function assignOrder(orderId) {
            const distributorSelect = document.getElementById(`distributor-${orderId}`);
            const distributorUsername = distributorSelect.value;
            ...
            if (confirm('Are you sure you want to assign this order to the selected distributor?')) {
                fetch('update_order_status.php', {
                    ...
                    .then(data => {
                        if (data.success) {
                            ...
                            alert('Error assigning order: ' + data.message);
                        }
                    })
                    .catch(error => {
